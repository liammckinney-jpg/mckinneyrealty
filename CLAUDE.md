...
- Coverage: **province-wide Ontario**. Never a count of markets. This is the firm's **service area** and it is unchanged. It is not a claim about MLS® data completeness — the PropTx IDX feed does not cover every Ontario board, the pool is opt-in at both brokerage and listing level, and IDX Data Agreement 6.3(j) forbids claiming full access to the MLS® System. Keep the two apart: we serve Ontario; the listings page does not show all of it.
...
- **Exception — listings module (`/apartment-buildings-for-sale`), granted by Liam, Sept 18 2026.** Public copy for these routes may be authored here. The reason: the real MLS® data model (PropTx Form 590) has no unit-count field and leaves income figures blank, which forced a rewrite of the spec's §6 strings that no longer had an author. The bar is unchanged — the hard content rules above, KB-02/KB-16 (R1 Plain-Meaning, R2 no interior vocabulary, R11 projected figures, R18 "apartment building"), and **no claim the feed cannot support**. Write the string into the spec's §6 in Notion first, then build to it verbatim. Liam approves before it reaches main.
...
- **No computed metric appears on these routes at all** (spec amendment Sept 18 2026). The MLS® commercial form has no unit-count field — buildings carry a four-value bucket (`Apts - 2 to 5 / 6 to 12 / 13 to 20 / Over 20 Units`) — so `pricePerUnit` cannot be calculated, and gross income / operating expense / NOI are optional fields normally left blank. `units`, `reported{}` and `derived{}` are gone from the schema. Property taxes are supplied and may be shown as given. Projection lives only in the Underwriter, with editable, disclosed assumptions.
- **100-listing cap (IDX Data Agreement 6.3(b)).** A consumer may view or retrieve no more than 100 listings per inquiry. Results are paginated server-side; never render the whole snapshot into one page.
- **Never map `REALTOR ONLY REMARKS`** — brokerage-only. `REMARKS FOR CLIENTS` is the public field, rendered as supplied.
- Honour the per-listing switches: `DISTRIBUTE TO DDF/IDX`, `DISPLAY ADDRESS ON INTERNET` (cards and detail H1 fall back to `{city}` alone — no substitute text), `PERMISSION TO CONTACT LISTING BROKER TO ADVERTISE`.
- **Never put IDX data into an AI system (IDX 6.2(k)).** Feed responses, listing records, snapshots and derived tallies are not pasted into Claude Code or any other AI tool. Write adapters against the published RESO schema; Liam relays observations in his own words. Open check C8 asks PropTx to scope this.
