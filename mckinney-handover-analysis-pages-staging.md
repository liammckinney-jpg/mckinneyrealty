...
- Mobile (coarse-pointer/UA): single Share button → OS sheet, which carries the social targets (FB, IG, LinkedIn, WhatsApp). Desktop: always the explicit row — Copy link (bare canonical, flips to "Copied" 2s) / Email / LinkedIn / Facebook — plus "More…" opening the OS sheet where available (macOS). UTM encoded inside the shared URL, new-tab + noopener, GA4 `share` event with method/content_type/item_id. No third-party anything; hidden in print. (Spec's `navigator.share`-exists branch alone was wrong on desktop — macOS passes it but its sheet has no social targets; corrected per Liam's review. Instagram has no web share URL — it is reachable only through the mobile OS sheet, which is why no IG button exists anywhere on the web.)
